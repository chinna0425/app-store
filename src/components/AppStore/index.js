...
class AppStore extends Component {
  state = {status: 'SOCIAL', searcher: ''}

  tabchanger = tabId => {
    this.setState({status: tabId})
  }
  ...
  render() {
    const {status, searcher} = this.state
    const latestlist = appsList.filter(items => items.category === status)
    const updates = latestlist.filter(latestitems =>
      latestitems.appName.toLowerCase().includes(searcher.toLowerCase()),
    )
    return (
      <div className="background-container">
        ...
        <ul className="unorderedlist1">
          {tabsList.map(eachitem => (
            <TabItem
              elements={eachitem}
              key={eachitem.tabId}
              isActive={eachitem.tabId === status}
        ...
        <ul className="unorderedlist2">
          {updates.map(eachset => (
            <AppItem setelement={eachset} key={eachset.appId} />
          ))}
        </ul>
      </div>
    )
